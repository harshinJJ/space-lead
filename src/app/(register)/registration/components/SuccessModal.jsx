...
import { ErrorIcon, PendingIcon, SuccessIcon } from "@/data/icons/statusIcons";
  ...
  switch (status) {
    case "1":
      return {
        ...
        description: "You will be notified once it's confirmed",
        icon: <PendingIcon />,
        className: "text-[#ec942c]",
      };
    case "2":
      return {
...
const SuccessModal = ({ onContinue, status, ticketUrl, uid, invoiceId }) => {
  ...
  const statusData = getMessage(status);
  return ticketUrl ? (
    <div className="flex flex-col items-center justify-center bg-white rounded-3xl shadow-lg p-8 py-14.5 w-full max-w-[44rem] mx-auto mt-12">
      ...
      <div className="relative">
        <button
          ref={shareRef}
          onClick={handleShare}
          className="relative flex items-center gap-3 bg-gradient-to-r from-[#1F273F] via-[#3D4762] to-[#432F5F] p-2 rounded-full mt-5"
        >
          <svg
            width="19"
            height="20"
    ...
    <div className="flex flex-col items-center justify-center bg-white rounded-3xl shadow-lg p-8 py-14.5 w-full max-w-[44rem] mx-auto mt-12">
      ...
      {/* Error Text */}
      <h2
        className={`text-[2rem] text-center text-[#E84C4C] ${statusData.className} mb-2`}
      >
        {statusData.title}
      </h2>
      <p className="text-[#22222280] tracking-[-2%] mb-4">
